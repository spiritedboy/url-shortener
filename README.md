...
| 连接池惰性保活 | MySQL / Redis 连接池空闲超 600 秒才做存活检测（PING），活跃连接零开销；失效自动重连 |
| 安全 | SQL 注入防护（mysql_real_escape_string）、XSS 防护（前端 HTML 转义）|
...
**连接池惰性保活**：
MySQL 和 Redis 连接池均记录每个连接的最后归还时间。`acquire()` 借出连接时，仅当空闲时间超过 600 秒才发送存活探测（MySQL `mysql_ping()`、Redis `PING`），检测失败则自动销毁并重建连接。高并发场景下活跃连接完全跳过检测，零额外开销。
